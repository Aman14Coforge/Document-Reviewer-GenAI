...
def dummy_call_llm_json(prompt: str) -> dict:
    """
    Dummy LLM response for local/UI testing without Model Garden credentials.
    Applies simple keyword heuristics per rule.
    """
...
def _extract_document_from_prompt(prompt: str) -> str:
    match = re.search(r"DOCUMENT CONTENT:\s*(.*)\Z", prompt, re.DOTALL)
    return match.group(1).strip() if match else ""
...
def call_llm_json(
    ...
    *,
    ...
    if use_dummy:
        return dummy_call_llm_json(prompt)
    raw = call_llm(
        prompt,
