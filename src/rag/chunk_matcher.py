...
from src.rag.rule_store import (
    load_compliance_config,
    resolve_rules_by_ids,
    retrieve_rule_ids_for_text,
)

logger = get_logger("rag.matcher")


def build_chunk_query_text(chunk: dict, *, max_chars: int = 1500) -> str:
    """Include section metadata in the query — improves keyword/section reranking."""
...
def match_rules_to_chunks(
    ...
    *,
    ...
    for chunk in chunks_data.get("chunks", []):
        ...
        query_text = build_chunk_query_text(chunk)
        retrieved_ids = retrieve_rule_ids_for_text(
            query_text,
            top_k=top_k,
    ...
    logger.info("RAG matched %s chunk(s), top_k=%s", len(matches), top_k)
    return {
