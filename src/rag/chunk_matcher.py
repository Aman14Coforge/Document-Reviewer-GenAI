...
def load_chunk_rag_config() -> dict:
    ...
    return config.get(
        "chunk_rag",
        {
            ...
            "fallback_to_all_rules_if_empty": True,
        },
    )
...
def match_rules_to_chunks(
    ...
    *,
    ...
    top_k = top_k or rag_config.get("top_k_rules_per_chunk", 5)
    always_include_rule_ids = always_include_rule_ids or rag_config.get("always_include_rule_ids", [])
    skip_chunk_ids = set(skip_chunk_ids or rag_config.get("skip_chunk_ids", ["full_document"]))
    fallback_all = rag_config.get("fallback_to_all_rules_if_empty", True)
